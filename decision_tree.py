...
class DecisionTree:
    ...
    def best_split(self, X, y):
        ...
        shuffled_indices = np.random.permutation(np.arange(len(self.features)))  # todo: You'd better use a random permutation of features 0 to D-1
        for index in shuffled_indices:
            if index not in self.used_features_indices:
                thresholds = np.unique(X[:, index]) # todo: use unique values in this feature as candidates for best threshold
...
if __name__=="__main__":
    ...
    # find the best_max_depth in range [1,num_features]
    # find the best_min_samples_split in range [2,10]
    num_samples, num_features = X.shape
    for max_depth in range(1,num_features+1):
        for min_samples_split in range(2,11):
            dt = DecisionTree(features=features, max_depth=max_depth, min_samples_split=min_samples_split)
    ...
    print(f"Best validation set accuracy: {best_val_acc}")
